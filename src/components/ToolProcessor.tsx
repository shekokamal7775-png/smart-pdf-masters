...
export function ToolProcessor({ slug }: Props) {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      toast.success(lang === "ar" ? "تمت المعالجة بنجاح!" : "Done! Your file is ready.");
    } catch (err: unknown) {
      ...
      >
        ...
        <div className="mt-4 flex flex-wrap justify-center gap-2">
          {cfg.accept.split(",").map((fmt) => (
            <span
              key={fmt}
              className="rounded-full border border-border bg-secondary/50 px-2.5 py-0.5 text-xs font-medium text-muted-foreground"
            >
